...
void TWI_start(void)
{
    ...
    /* Wait for acknowledgment flag */
    while(TWI_getStatus() != TW_START);
}
 ...
 *
 * [Args]:	  au8_data, enu_choice
 *
 * [in]		  au8_data: Unsigned Character (data need to be written in the TWI)
 * 		  enu_choice: Enumerator that determine choice (data, address)
 *
 * [out]	  None
 *
 * [in/out]	  None
 ...
 * [Returns]:     None
 *****************************************************************************************/
void TWI_write(uint8_t au8_data, enuTWI_WriteChoice_t enu_choice)
{
    ...
    if(enu_choice == ADDRESS)
    {
        /* Wait for acknowledgment flag */
        while(TWI_getStatus() != TW_MT_SLA_W_ACK);
    }
    else if(enu_choice == DATA)
    {
        /* Wait for acknowledgment flag */
